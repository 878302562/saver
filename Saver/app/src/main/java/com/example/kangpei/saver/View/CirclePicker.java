...
/**
 * Created by Robin on 2016/11/22.
 */
...
public class CirclePicker extends View
{
    ...
    private Handler handler = new Handler()
    {
        ...
        public void handleMessage(Message msg)
        {
            ...
            {
                mCurrentRadian -= (2 * Math.PI) / 3600;//Robin
                mCurrentTime--;
                if (mCirclePickerListener != null)
                {
                    mCirclePickerListener.onPickerpickingValueChanged(mCurrentTime);
                }
            }
            else
            {
                ...
                if (mCirclePickerListener != null)
                {
                    mCirclePickerListener.onPickerStop();
                }
            }
            invalidate();
        }
    ...
    // Runt
    private CirclePickerListener mCirclePickerListener;
...
//        canvas.drawColor(Color.RED);
       ...
       // Log.d(TAG, "onDraw");
        ...
        canvas.drawText("100", mCx, getMeasuredHeight() / 2 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
                mLongerLineLength + mGapBetweenNumberAndLine + getFontHeight(mNumberPaint), mNumberPaint);
        canvas.drawText("25", mCx + mRadius - mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine - mLongerLineLength -
                textLength / 2
                - mGapBetweenNumberAndLine, mCy + getFontHeight(mNumberPaint) / 2, mNumberPaint);
        canvas.drawText("50", mCx, getMeasuredHeight() / 2 + mRadius - mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine -
                mLongerLineLength - mGapBetweenNumberAndLine, mNumberPaint);
        canvas.drawText("75", getMeasuredHeight() / 2 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
...
//        canvas.drawText(":", mCx, mCy + getFontHeight(mTimerNumberPaint) / 2, mTimerColonPaint);
        canvas.drawText(mCurrentTime+"",mCx,mCy + getFontHeight(mTimerNumberPaint) / 2, mTimerColonPaint);
        canvas.restore();
        // Timer Text
        canvas.save();
        canvas.drawText("Reminder Setting", mCx, mCy + getFontHeight(mTimerNumberPaint) / 2 + mGapBetweenTimerNumberAndText + getFontHeight
...
//                    Log.d(TAG, "In circle button");
            ...
            case MotionEvent.ACTION_MOVE:
                if (mInCircleButton && isEnabled())
                {
                    ...
                    if (mCirclePickerListener != null)
                        mCirclePickerListener.onTimerSetValueChange(getCurrentTime());
                    mCurrentTime = (int) (100 / (2 * Math.PI) * mCurrentRadian);
                    //Log.d("mcurrentRadian",100*mCurrentRadian/(2 * Math.PI)+"");
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
                if (mInCircleButton && isEnabled())
                {
                    ...
                    if (mCirclePickerListener != null)
                        mCirclePickerListener.onPickerSetValueChanged(getCurrentTime());
                }
                break;
        }
        return true;
...
//                    Log.d(TAG, "TimerTask");
            ...
            mStarted = true;
            if (this.mCirclePickerListener != null)
            {
                this.mCirclePickerListener.onPickerStart(mCurrentTime);
            }
        }
    }
    ...
    public void pauseTimer()
    {
        if (mStarted)
        {
            ...
            if (this.mCirclePickerListener != null)
            {
                this.mCirclePickerListener.onPickerPause(mCurrentTime);
            }
        }
    }
    ...
    {
        if (time >= 0 && time <= 3600)//Robin
        {
            mCurrentTime = time;
            if (mCirclePickerListener != null)
            {
                mCirclePickerListener.onPickerSetValueChanged(time);
            }
     ...
     * set timer listener
     *
     * @param mCirclePickerListener
     */
    public void setPickerListener(CirclePickerListener mCirclePickerListener)
    {
        this.mCirclePickerListener = mCirclePickerListener;
    }
    ...
    public interface CirclePickerListener
    {
         ...
         * @param time
         */
        void onPickerpickingValueChanged(int time);
         ...
         * @param time
         */
        void onPickerSetValueChanged(int time);
